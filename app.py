"""Ward — a private, table-side control surface for D&D Dungeon Masters.

Ward supports an in-person game without trying to simulate it. The physical
table is authoritative; the map is a quick spatial note, while initiative,
HP, conditions, reminders, references, and campaign continuity stay close at
hand.

Run:  ward  (or .venv/bin/python app.py from a checkout)
"""
...
def _offline_mode() -> bool:
    """Use Ward's public name while honoring the former variable for compatibility."""
...
class BattleApp(App[None]):
    ...
    def _saved_campaigns(self, data: dict) -> list[tuple[str, int]]:
        ...
        return sorted(campaigns, key=lambda item: (item[0] != active, item[0].casefold()))
    ...
    def _write_ward_backup(self) -> str:
        if self._session_started:
            self._remember_current_encounter()
        return ward_backup.write_backup(
            BACKUP_DIR,
            self._campaigns_read(),
            self._encounters_read(),
            self._templates_read(),
        )

    async def _ward_data_flow(self) -> bool:
        """Back up or restore all user-owned Ward data.

        Returns True when a restore replaced the live session.
        """
        while True:
            backups = ward_backup.list_backups(BACKUP_DIR)
            options = [("backup", "Back up Ward data\n  [dim]Campaigns, encounters, parties, notes, and monster setups[/]")]
            if backups:
                options.append(("restore", f"Restore a backup\n  [dim]{len(backups)} available · current data is backed up first[/]"))
            options.append(("back", "Back"))
            picked = await self.push_screen(ListModal("WARD DATA", options, wide=True), wait_for_dismiss=True)
            ...
            choices = [
                (path, f"{escape(os.path.basename(path))}\n  [dim]{os.path.getsize(path):,} bytes[/]")
                for path in backups
            ]
            choices.append(("back", "Back"))
            path = await self.push_screen(ListModal("RESTORE WARD DATA", choices, wide=True), wait_for_dismiss=True)
            ...
            confirmed = await self.push_screen(
                ListModal(
                    "RESTORE THIS BACKUP?",
                    [
                        ("restore", "Restore campaigns and encounters\n  [dim]Ward will first preserve the current data in a new backup[/]"),
            ...
            restored = self._last_encounter_record()
            if restored is not None:
                self._resume_current_encounter(restored)
            else:
                self._setup()
                self._session_started = False
    ...
    def _startup_menu(
        ...
        """Build an adaptive folio around preparing, running, and resuming."""
        ...
        subtitle = (
            ...
            "No active campaign · Ward can still run a standalone encounter"
        )
        return options, "Resume, run, or prepare", subtitle
    ...
    async def _boot_campaign(self) -> None:
        ...
        while True:
            ...
            backup_count = len(ward_backup.list_backups(BACKUP_DIR))
            options, prompt, subtitle = self._startup_menu(data, current, backup_count)
            ...
            if choice == "sample":
                ...
                return
            if choice and choice.startswith("run:"):
                if await self._new_named_encounter_flow(choice[4:]):
                    return
                continue
            if choice and choice.startswith("prepare:"):
                if await self._campaign_home_flow(choice[8:]):
                    return
                continue
            if choice == "data":
    ...
    async def _campaign_home_flow(self, name: str) -> bool:
        ...
        while True:
            ...
            picked = await self.push_screen(
                ListModal(
                    f"WARD · {escape(name)} · {party_size} ADVENTURER{'S' if party_size != 1 else ''}",
                    options,
                    wide=True,
                ),
                wait_for_dismiss=True,
    ...
    async def _choose_campaign_flow(self) -> str | None:
        ...
        if self._session_started and any(c.kind == "PC" for c in self.combatants):
            options.append(("new_from_table", "+ Create a campaign from the party on the table"))
        options.append(("back", "Back"))
        ...
        if picked == "new_from_table":
            return await self._new_campaign_from_current_party_flow()
        return None

    async def _campaign_details_flow(self, name: str) -> None:
        while True:
            data = self._campaigns_read()
            campaign = data.get("campaigns", {}).get(name)
            if not isinstance(campaign, dict):
            ...
            picked = await self.push_screen(
                ListModal(
                    f"CAMPAIGN DETAILS · {escape(name)}",
                    [
                        ("party", f"Party · {party_size} adventurer{'s' if party_size != 1 else ''}"),
                        ("notes", f"Session notes · {'saved' if campaign.get('notes') else 'empty'}"),
                        (
                            "ruleset",
                            f"Rules reference · {escape(ruleset)}\n"
                            "  [dim]Used only to guide lookups; Ward does not enforce rules[/]",
                        ),
    ...
    async def _new_campaign_flow(self, *, first_run: bool = False) -> str | None:
        ...
        self._log(f"Campaign '{escape(name)}' created. Ward will use the {DEFAULT_RULESET} reference for lookups.", kind="import")
        return name
    ...
    async def _edit_campaign_party_flow(self, name: str, *, first_run: bool = False) -> str | None:
        ...
        campaign = data.setdefault("campaigns", {}).setdefault(name, {})
        if not isinstance(campaign, dict):
            campaign = {}
    ...
    async def _edit_campaign_ruleset_flow(self, name: str) -> None:
        ...
        picked = await self.push_screen(
            ListModal(
                f"RULES REFERENCE · {escape(name)}",
                [
                    ("2024", f"2024 reference{'  [green]✓[/]' if current == '2024' else ''}"),
                    ("2014", f"2014 reference{'  [green]✓[/]' if current == '2014' else ''}"),
                ],
            ),
            wait_for_dismiss=True,
        )
    ...
    async def _new_campaign_from_current_party_flow(self) -> str | None:
        name = await self.push_screen(
            TextModal("CREATE CAMPAIGN", "campaign name (e.g. Lost Mine)", confirm="Create"),
            wait_for_dismiss=True,
        )
        if not name:
            return None
    ...
    def _party_context(self) -> str:
        ...
        if not pcs:
            return "No PCs loaded; report encounter pressure as Unknown."
        levels = [c.level for c in pcs if c.level is not None]
        hp_total = sum(max(0, c.max_hp) for c in pcs)
        hp_current = sum(max(0, c.hp) for c in pcs)
        acs = [c.ac for c in pcs]
        profs = [c.proficiency for c in pcs if c.proficiency is not None]
        spell_dcs = [c.spell_dc for c in pcs if c.spell_dc is not None]
        attack_bonuses = []
        for c in pcs:
            for attack in c.attacks:
                match = re.search(r"\s([+-]\d+)\s*·", attack)
                if match:
                    attack_bonuses.append(int(match.group(1)))
        roles = ", ".join(c.role for c in pcs if c.role) or "roles unknown"
        level_text = ", ".join(str(level) for level in levels) if levels else "unknown"
        attack_text = (
            f"+{min(attack_bonuses)} to +{max(attack_bonuses)}"
            if attack_bonuses else "unknown"
        )
        return (
            f"{len(pcs)} PC{'s' if len(pcs) != 1 else ''}; levels {level_text}; "
            f"total HP {hp_current}/{hp_total}; AC {min(acs)}-{max(acs)} (avg {sum(acs) / len(acs):.1f}); "
            f"proficiency +{min(profs) if profs else 'unknown'} to +{max(profs) if profs else 'unknown'}; "
            f"attack bonuses {attack_text}; spell DCs {', '.join(map(str, spell_dcs)) or 'none'}; "
            f"roles: {roles}"
        )

    async def _ai_encounter_flow(self) -> None:
        description = await self.push_screen(
            TextModal("ENCOUNTER ASSISTANT", "tense undead crypt encounter", confirm="Suggest"),
            wait_for_dismiss=True,
        )
        if not description:
            return
        ...
        for name, count, (source, data) in resolved:
            for _ in range(count):
                ...
                if result:
                    added += 1
        if added:
            self._log(f"Suggested encounter added — {added} of {total} monsters ready; initiative unset.", kind="import")

    async def _encounter_templates_flow(self) -> None:
        data = self._templates_read()
        options = [
            (f"load:{name}", f"[bold #a8d0ff]{escape(name)}[/]")
            for name in sorted(data.get("templates", {}))
        ]
        options.append(("save", "Save these monsters as a prepared encounter"))
        picked = await self.push_screen(ListModal("PREPARED ENCOUNTERS", options), wait_for_dismiss=True)
        if not picked:
            return
        if picked == "save":
    ...
    async def _choose_prepared_encounter(self) -> str | None:
        data = self._templates_read()
        options = [
            (f"load:{name}", escape(name))
            for name in sorted(data.get("templates", {}))
        ]
        if not options:
            await self.push_screen(
                ListModal("PREPARED ENCOUNTERS", [("back", "No prepared encounters yet · Esc to go back")]),
                wait_for_dismiss=True,
            )
            return None
        picked = await self.push_screen(ListModal("PREPARED ENCOUNTERS", options), wait_for_dismiss=True)
        return picked[5:] if picked and picked.startswith("load:") else None

    async def _save_template_flow(self) -> None:
        monsters = [c for c in self.combatants if c.kind != "PC"]
        if not monsters:
            self._log("Add monsters before saving a prepared encounter.", kind="warn")
            return
        name = await self.push_screen(
            TextModal("SAVE PREPARED ENCOUNTER", "name (e.g. Goblin Ambush)", confirm="Save"),
            wait_for_dismiss=True,
        )
        if not name:
            return
    ...
    def _load_template(self, name: str, data: dict, record_undo: bool = True) -> bool:
        ...
        party = [self._combatant_snap(c) for c in self.combatants if c.kind == "PC"]
        for pc in party:
            pc["init"] = None
            pc["conditions"] = []
        monsters = [
            ...
            for creature in snap["combatants"]
            if isinstance(creature, dict) and creature.get("kind") != "PC"
        ]
        merged["combatants"] = party + monsters
        merged["round"] = 1
        merged["turn"] = None
        merged["sel"] = len(party) if monsters else (0 if party else None)
        merged["moving"] = False
        ...
        moved = False
        for creature in self.combatants:
            if any((other.x, other.y) == (creature.x, creature.y) for other in placed):
                spot = find_free_spot(placed, MAP_COLS, MAP_ROWS)
                if spot is not None:
                    creature.x, creature.y = spot
                    moved = True
            placed.append(creature)
        if moved:
            self._refresh_all()
    ...
    def _remember_current_encounter(self) -> bool:
        """Atomically remember the live battlefield without interrupting play."""
    ...
    def action_save(self) -> None:
        if self._remember_current_encounter():
            self._log("Encounter remembered.", kind="import")
        else:
    ...
    async def _browse_flow(self) -> None:
        ...
        if srd_data is None and not _offline_mode():
    ...
    async def _spell_flow(self) -> None:
        ...
        if spells is None and not _offline_mode():
    ...
    async def _edit_flow(self) -> None:
        ...
        if picked == "name":
            ...
            val = await self.push_screen(TextModal("EDIT ROLE", c.role, confirm="Apply"), wait_for_dismiss=True)
            if val and val != c.role:
                do(lambda: setattr(c, "role", val))
        elif picked == "note":
            val = await self.push_screen(TextModal("EDIT NOTE", c.note, confirm="Apply"), wait_for_dismiss=True)
            if val and val != c.note:
                do(lambda: setattr(c, "note", val))
        elif picked == "reminder":
            val = await self.push_screen(TextModal("EDIT TURN REMINDER", c.reminder, confirm="Apply"), wait_for_dismiss=True)
            if val != c.reminder:
    ...
    async def _new_encounter_flow(self) -> None:
        campaign = self._session_campaign
        destination = (
            f"Start a new encounter in {escape(campaign)}"
            if campaign else "Start without a campaign"
        )
        if self.combatants:
            options = [
